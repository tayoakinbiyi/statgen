...
def qqPlotsAll(traitChr,scratchDir,chrs): 
    ...
    axsQQ.set_xlim([0,mMax])
    axsQQ.set_ylim([0,mMax])
    axsQQ.plot(-np.log(cisX),-np.log(cis),'ro',label='cis')
    axsQQ.plot(-np.log(transX),-np.log(trans),'bo',label='trans')
    axsQQ.set_ylabel(traitChr)
    axsQQ.legend()
    axsQQ.plot(axsQQ.get_xlim(), axsQQ.get_ylim(), ls="--", c=".3")
    
    figQQ.savefig(scratchDir+'qq_all_'+traitChr+'.png',bbox_inches='tight')
    plt.close('all')

    
chrs=['chr'+str(x) for x in range(1,20)]
for traitChr in chrs:
    qqPlotsAll(traitChr,scratchDir,chrs)
