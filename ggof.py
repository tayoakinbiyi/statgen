...
def makeProb(z,t_mu,t_eps,lam,gamma,ar,cr):
    N=z.shape[1]
    ...
    for mu in t_mu:
        for eps in t_eps:
            if mu*eps>0:
                ...
                #    results=executor.map(ebb_gnull, [(N,arr,cr,z[i*int(np.ceil(H/M)):min((i+1)*int(np.ceil(H/M))),non_zero])
                    
def mp(d,z,ar,cr,pairwise_cors):
    non_zero = np.array(range(int(np.ceil(d/2.0))))
    
    lam = np.linspace(0,.5,1000) #np.array(range(d),dtype='float')
    gamma = np.append(np.linspace(0,.01,1000),np.linspace(.01,.14,1000)) #.append(npnp.array(range(d),dtype='float')
    k=range(1,int(np.ceil(d/2.0)))
    
    
    for t_z in z:
        ebb+=[1-sum(np.exp(cr[0:(k+1)]+np.nansum(np.log(ar[0:(k+1)]*gamma+1-lam),axis=1)+
...
def ebb_gnull1(x,d,ar,cr):
    k=int(x[0])
    lam=x[1]
    gamma=x[2]

    baseOne=np.append(np.array([0]),np.log(lam+np.array(range(0,k))*gamma))
    baseOne=np.cumsum(baseOne)
    
    baseTwo=np.log(np.array(range(d))*gamma+1-lam)
    baseTwo=np.sum(baseTwo[0:(d-k-1)])+np.cumsum(baseTwo[(d-k-1):])[::-1]
    
    baseThree=np.sum(np.log(1+np.array(range(d))*gamma))
    return(1-sum(np.exp(cr[0:(k+1)]+baseOne+baseTwo-baseThree)))
...
def ggof(z_raw,p,pairwise_cors,arr,cr,Types):
    ...
    #print(min(gamma[non_zero]),max(gamma[non_zero]))
    ...
    times+=[time.time()]
    cor['ggnull'] = -np.apply_along_axis(ebb_gnull1,1, param,d=d,ar=arr,cr=cr)
    times+=[time.time()]
    zz=-np.apply_along_axis(ebb_gnull,1, param,ar=arr,cr=cr)
    #tmp = -np.apply_along_axis(ebb_gnull1,1, param,d=d,ar=arr,cr=cr)#
    #pdb.set_trace()
    ...
    print([int(1000*(x-times[0])/(times[-1]-times[0])) for x in times])
