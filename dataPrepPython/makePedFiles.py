...
def makePedFiles(parms):
    ...
    traits,traitData,snps,snpData,mouseIds=initSnpTraits(parms,snps=True)
    ...
    pd.concat([pd.DataFrame({'Family ID':mouseIds,'Individual ID':0}),preds],axis=1).to_csv(
    ...
    makeTraitPedFiles(traits,traitData,mouseIds,parms)
    ...
    for snp in snpChr:  
        makeGrm(parms,snp,mouseIds,fastGrm,False)
    
    makeGrm(parms,'all',mouseIds,fastGrm,False)
