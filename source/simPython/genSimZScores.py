...
def genSimZScores(parms):
    ...
    for trait in traitChr:
        Y+=[np.loadtxt('ped/Y-'+str(trait)+'.txt',delimiter='\t')]
    ...
    for k in range(M):
        ...
        with ProcessPoolExecutor(numCores) as executor:
            ...
            while snp<snpSize[-1]:
                futures=[]
                for core in range(min(snpSize[-1]-snp,numCores)):
                    #genSimZScoresHelp(parms,mu,eps,core,snp,numSnpChr+k*numCores+core+1,
                    #    numTraitChr+k*numCores+core+1,snpSet.iloc[:,6+snp],Y,maf[snp],minor[snp],fastlmm)
        ...
        for trait in traitChr:
            z=np.loadtxt('score/waldStat-'+str(len(snpSize))+'-'+str(trait),delimiter='\t')
            ...
            np.savetxt('score/waldStat-'+str(len(snpSize)+1+k)+'-'+str(trait),z,delimiter='\t')
                 
        print('wrote '+str(mu)+' '+str(eps),flush=True)
                
    return()

def genSimZScoresHelp(parms,mu,eps,core,snpInd,snp,trait,snpVec,Y,maf,minor,fastlmm):
    local=parms['local']
    ...
    loc=np.random.choice(Y.shape[1],size=eps,replace=False)
    fam=pd.DataFrame({'Family ID':range(len(Y)),'Individual ID':0,'Paternal ID':range(len(Y)),'Maternal ID':0,'Sex':1,'Phenotype':1})
    ...
    DBLog(str(snp)+'\t'+str(mu)+'\t'+str(eps)+'\t'+str(eqtlList['z'].min())+'\t'+str(eqtlList['z'].max()))
