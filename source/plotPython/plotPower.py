...
def plotPower(parms):
    ...
    fig, axs = plt.subplots(len(muEpsRange),1,dpi=50)   
    ...
    for ind in range(len(muEpsRange)):                
        ...
        for TypeInd in range(len(Types)):
            ...
            pvals=np.sort(np.loadtxt('pvals/'+Type+'-'+str(snp),delimiter='\t'))
            
            DBLog(Type+'-'+str(snp)+' len(pvals) '+str(len(pvals))+' minP '+str(min(pvals))+' maxP '+str(max(pvals)))
            df.loc[:,Type]=-np.log10(np.clip(pvals,np.nextafter(0,1),1))
    ...
    pd.DataFrame(exact,columns=['scenario','Type','.05','.01']).to_csv('diagnostics/exact.csv',index=False)
