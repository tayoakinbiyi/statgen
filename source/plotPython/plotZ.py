...
def plotZ(z,prefix='',myZip=None):
    ...
    myQQ(x,y,prefix+title,myZip=myZip)
    ...
    myHist(np.corrcoef(z,rowvar=False)[np.triu_indices(numTraits,1)],'between trait',myZip=myZip)    
    myHist(np.corrcoef(z,rowvar=True)[np.triu_indices(numSnps,1)],'between snp',myZip=myZip)
