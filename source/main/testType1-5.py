...
zDat3=np.concatenate([np.loadtxt('score/waldStat-3-'+str(x),delimiter='\t') for x in traitChr],axis=1)[0:208]
zDat2=np.concatenate([np.loadtxt('score/waldStat-2-'+str(x),delimiter='\t') for x in traitChr],axis=1)[0:208]
zDat1=np.concatenate([np.loadtxt('score/waldStat-1-'+str(x),delimiter='\t') for x in traitChr],axis=1)[0:208]
zDatI=norm.rvs(size=zDat1.shape)[0:208]
#zRef=norm.rvs(size=[int(1e6),int(zDat1.shape[1])])
#Y=np.loadtxt('ped/Y.txt',delimiter='\t')

numSnps,N=zDat1.shape
print('num snps obs ',len(zDat1))
#######################################################################################################
...
for i in range(len(nm)):
    ...
    axs.set_title('traitMean-'+str(nm[i])+'-'+str(np.std(y)))
    ...
    mMin=min(np.min(y),np.min(x))
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('traitMeanOld-'+str(nm[i]))
    fig.savefig('diagnostics/traitMeanOld '+str(nm[i])+'.png')
    plt.close('all') 
    ...
    mMin=min(np.min(y),np.min(x))
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('snpMean-'+str(nm[i])+'-'+str(np.std(y)))
    ...
    mMin=min(np.min(y),np.min(x))
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('snpMeanOld-'+str(nm[i]))
    fig.savefig('diagnostics/snpMeanOld '+str(nm[i])+'.png')
    plt.close('all') 
    ...
    mMin=min(np.min(y),np.min(x))
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('eta '+str(nm[i])+'-mu: '+str(np.mean(y))+'-std: '+str(np.std(y)))
    ...
    mMin=min(np.min(y),np.min(x))
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('etaOld '+str(nm[i])+'-mu: '+str(np.mean(y))+'-std: '+str(np.std(y)))
    fig.savefig('diagnostics/traitMeanOld z^2 '+str(nm[i])+'.png')
    plt.close('all') 
    ...
    axs.plot([mMin,mMax], [mMin,mMax], ls="--", c=".3")   
    axs.set_title('etaOld '+str(nm[i])+'-mu: '+str(np.mean(y))+'-std: '+str(np.std(y)))
    fig.savefig('diagnostics/snpMeanOld z^2 '+str(nm[i])+'.png')
    plt.close('all') 
