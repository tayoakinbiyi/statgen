...
def myMain(parms):
    ...
    plotCorr(vY,'vY')
    myHist(vY[np.triu_indices(parms['numTraits'],1)],'vY-hist')
    ...
    L=makePSD(vZ)
    plotCorr(vZ,'vZ')
    offDiag=vZ[np.triu_indices(parms['numTraits'],1)]    
    ...
    zDep=np.matmul(norm.rvs(size=[int(1e5),parms['numTraits']]),L.T)
    zIndep=norm.rvs(size=[int(1e5),parms['numTraits']])
    
    #######################################################################################################
    #######################################################################################################
    #######################################################################################################
        
    stat=ELL.ell.ell(int(.3*parms['numTraits']),parms['numTraits'],qList=[0.1,0.05,0.01,0.001])
