...
def setupELLSnp(L,parms):
    ...
    with ProcessPoolExecutor(numCores) as executor:    
        for core in range(numCores):
            ...
            if len(binRange)==0:
                continue
            futures+=[executor.submit(getGamma,bins[binRange],rhoBar)]
...
def getGamma(binEdge,rho):
    z=np.abs(norm.ppf(binEdge/2))
    odds=getHerm(z,rho)  
    x=4*norm.pdf(z)**2*odds    
    return(pd.DataFrame({'binEdge':binEdge,'gamma':x/(binEdge*(1-binEdge)-x)}))
...
def setupELLHelp(N,core,minMaxKPerBin):
    ...
    pd.concat(ebb,axis=0).to_csv('ELL/ebb-'+str(core),index=False,sep='\t')
